...
If `docs/ai/` exists but the stub files are still empty (no real CONTEXT, no plans, README still says brain_mcp), this is a fresh template. Before doing any task:

1. Ask the user for project name, description, and what they're building.
2. Offer to run the renaming step (replace `brain_mcp` placeholders) or provide the commands.
